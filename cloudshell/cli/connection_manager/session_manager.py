...
from qualipy.common.libs.utils import normalizeBuffer
...
class SessionManager:
    ...
    def _readStartUpBuffer(self, expected_str, timeout):
        output = self.expect(expected_str, timeout)
        self._logger.info(output)
        if output == -1:
            output = self.hardwareExpect('', expected_str, timeout)

        return output

    def setUnsafeMode(self, state):
    ...
    def hardwareExpect(self, cmd, expected_str, timeout=30, expected_map={}, retry_count=10):
        ...
        for retry in range(retry_count):
            # self._logger.info("Retry: {0}".format(str(retry)))
    ...
    def expect(self, re_strings='', timeout=None):
        ...
        if not len(re_strings) == 0 and not len(found_pattern) == 0 and \
            ...
            # print(return_list)
            return [found_pattern[0][0], normalizeBuffer(current_output)]
        else:
            # We would socket timeout before getting here, but for good
            # measure, let's send back a -1
            return [-1, normalizeBuffer(current_output)]

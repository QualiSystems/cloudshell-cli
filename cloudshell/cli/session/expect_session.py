...
from cloudshell.cli.service.action_map import ActionLoopDetector, ActionMap
...
class ExpectSession(Session, metaclass=ABCMeta):
    ...
    def hardware_expect(
        ...
        **optional_args
        ...
        :param bool remove_command_from_output: In some switches the output string
            includes the command which was called. The flag used to verify whether
             the command string removed from the output string.
        :rtype: str
        """
        if not action_map:
        ...
        while retries == 0 or retries_count < retries:
            ...
            action_matched = action_map.process(
                session=self,
                logger=logger,
                output=output_str,

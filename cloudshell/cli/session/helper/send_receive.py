...
def send_line(session: "Session", command: str, new_line: str = "\r") -> None:
    """Add new line to the end of command string and send."""
    session.send(command + new_line)


def receive_all(session: "Session", timeout: int = 30) -> str:
...
def clear_buffer(session: "Session", timeout: Optional[int] = None) -> str:
    """Clear buffer."""
    out = ""
    if not timeout:
...
def check_active(session: "Session", timeout: int = 1) -> bool:
    send_line(session, "")

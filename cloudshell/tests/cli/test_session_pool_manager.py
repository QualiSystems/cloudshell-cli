...
class TestSessionPoolManager(TestCase):
    ...
    def test_remove_session_call(self):
        session = Mock()
        self._session_pool_manager.remove_session(session, self._logger)
        self._session_manager.remove_session.assert_called_once_with(session, self._logger)
    ...
    def test__new_session_called(self):
        prompt = Mock()
        session = self._session_pool_manager._new_session(self._new_sessions, prompt, self._logger)
        self._session_manager.new_session.assert_called_once_with(self._new_sessions, prompt, self._logger)
    ...
    def test__get_from_pool_called_get(self):
        prompt = Mock()
        self._session_pool_manager._get_from_pool(self._new_sessions, prompt, self._logger)
    ...
    def test__get_from_pool_is_compatible_called(self):
        ...
        self._session_pool_manager._get_from_pool(self._new_sessions, prompt, self._logger)
        self._session_manager.is_compatible.assert_called_once_with(session, self._new_sessions, self._logger)
    ...
    def test__get_from_pool_remove_called(self):
        ...
        self._pool.get.return_value = session
        self._session_pool_manager._get_from_pool(self._new_sessions, prompt, self._logger)
        self._session_pool_manager.remove_session.assert_called_once_with(session, self._logger)
    ...
    def test__get_from_pool_new_session_called(self):
        ...
        self._pool.get.return_value = session
        self._session_pool_manager._get_from_pool(self._new_sessions, prompt, self._logger)
        self._session_pool_manager._new_session.assert_called_once_with(self._new_sessions, prompt, self._logger)
